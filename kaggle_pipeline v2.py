...
DATA_ROOT     = Path("/kaggle/input/rsna-knee-abnormality-2024") if IS_KAGGLE else Path("C:/kabir/RSNA_Knee_AI/DATA")
...
def _find_medsiglip():
    ...
    print("  Scanning /kaggle/input for MedSigLIP...")
...
# unzipped on disk. Only the studies we actually pick to train on get
# extracted out of it, one time — re-runs just find the folders and skip.
...
# STEP 1 — SCAN DICOMS
# ══════════════════════════════════════════════════════════════════════════════
def scan_dicoms(series_root, series_csv):
    """Scan all DCMs, join with series CSV for slot metadata."""
    series_root = Path(series_root)
    paths = list(series_root.rglob("*.dcm"))
    print(f"  DCM files found: {len(paths)}")

    rows = []
    for p in tqdm(paths, desc="  Reading headers"):
        try:
            ...
            if study and series:
                rows.append(dict(filepath=str(p), StudyInstanceUID=study,
...
def auc_mean(y_true, y_pred):
    ...
    for i in range(len(TARGETS)):
        if len(np.unique(y_true[:, i])) > 1:
            vals.append(roc_auc_score(y_true[:, i], y_pred[:, i]))
...
def train_fold(train_ds, val_ds, device, epochs):
    ...
    for epoch in range(epochs):
        ...
        for i in np.random.permutation(len(train_ds)):
            ...
            x, mask, idx, y = x.to(device), mask.to(device), idx.to(device), y.to(device)
            opt.zero_grad()
...
def main():
    ...
    print("\n── TRAIN: Scan DICOMs ──")
    train_dcm = scan_dicoms(TRAIN_SERIES, train_series_csv)
    train_dcm.to_csv(WORK_DIR / "train_dicom_index.csv", index=False)
    ...
    # simple train/val split for pretraining (val just used to pick best
    # weights across epochs, not for reporting accuracy)
    rng = np.random.RandomState(SAMPLE_SEED)
    perm = rng.permutation(len(pretrain_ids))
    n_val = max(1, int(0.1 * len(pretrain_ids)))
    val_ids = pretrain_ids[perm[:n_val]]
    tr_ids  = pretrain_ids[perm[n_val:]]

    pre_tr_ds = StudyDataset(emb[emb["StudyInstanceUID"].isin(tr_ids)],
                              lbl[lbl["StudyInstanceUID"].isin(tr_ids)])
    pre_va_ds = StudyDataset(emb[emb["StudyInstanceUID"].isin(val_ids)],
                              lbl[lbl["StudyInstanceUID"].isin(val_ids)])
    print(f"Stage A train={len(pre_tr_ds)}  val={len(pre_va_ds)}")

    pretrained_model = train_fold(pre_tr_ds, pre_va_ds, device, epochs=30)
    ...
    torch.save({"model_state_dict": pretrained_model.state_dict(),
               ...
               pretrain_path)
    print(f"Saved Stage A weights: {pretrain_path}")
    ...
    for fold, (tri, vi) in enumerate(gkf.split(table, groups=table.study), 1):
        ...
        fold_model.load_state_dict(pretrained_model.state_dict())
